...
//biến cho pwm, pid
int a=1.2, b=-1.2;//biến quy định khoảng cân bằng 1 -1,2 -2,
long pe,p,i,d ;
float kp=45, ki=1, kd=15; //100 0 10,100 0 15,30 1 15
float pre_pwmVal=0,_pV=0;
...
//  Serial.println("");
  ...
  //setMotor(1, 250,ENA, ENB, EN1,EN2,EN3,EN4 );
  _pV=goc_vantoc_right(a.acceleration.x,a.acceleration.y)-pre_pwmVal;
  for(int i=0;i<(int)abs(_pV);i++){
    //(_pV-pre_pwmVal)/abs(_pV)*i + pre_pwmVal
    setMotor_right(dir(a.acceleration.y),(float)(_pV-pre_pwmVal)/abs(_pV)*i + pre_pwmVal,ENA,EN1,EN2);//cả hai bánh đang cùng 1 vận tốc
    setMotor_left(dir(a.acceleration.y),(float)(_pV-pre_pwmVal)/abs(_pV)*i + pre_pwmVal,ENB,EN3,EN4);
  }
  pre_pwmVal=pre_pwmVal+_pV;
  //setMotor_right(dir(a.acceleration.y),goc_vantoc_right(a.acceleration.x,a.acceleration.y)-oset,ENA,EN1,EN2);//cả hai bánh đang cùng 1 vận tốc
  //setMotor_left(dir(a.acceleration.y),goc_vantoc_left(a.acceleration.x,a.acceleration.y),ENB,EN3,EN4);
  Serial.print("direction ");
  Serial.print(dir(a.acceleration.y));
  Serial.println(" ");
  Serial.print("vantoc ");
  Serial.print(vantoc());
  Serial.println("");
