...
long pe,p,i,d ;
float kp=55, ki=1, kd=7; //100 0 10,100 0 15,30 1 15,55 1 15,55 1 14
...
float PID(float error, float I, float D )
{
  ...
  float P,PID_value;
    ...
    I = I + error;
    D = error - pe;
    if(error>=15)return 255;//15
    else if (error<15) PID_value =500*(1+(0.06*0.5*sin(error)*pow(error,2))/2-(3*cos(error)*0.06*9.6*sin(error))/7)/(1.2+0.06-(3*0.06*pow(cos(error),2))/7) ;
  map(PID_value,255,220,220,255);;//318.417
