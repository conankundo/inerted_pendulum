...
float Ts = 50; // ms, khoảng tg tính vận tốc
...
void vantoc()
{
  ...
  if(dung == false){
    ...
    if (digitalRead(ENC2_1)!=0) count++;
    thoigian=millis()-time_begin;
    
  //tính toán
    if(thoigian>Ts){
     so_vong = count / do_phan_giai;
     tanso = so_vong * 1000 / thoigian;
     Serial.print("Numbers of Pulse: ");
     Serial.println(count);
     count = 0;
     dung = false;
     Serial.print("Speed: ");
     Serial.println((float)tanso*R*2*Pi);     
  }
}
...
//  d=p-pe;
  ...
  if (gocHt<=70){
  if (y > 1  || y < -1 ) {
//    vt = gocHt*gocHt*gocHt/3   ;
    vt=HS*
  }
  //if (y > 1  || y < -1 ) vt = PID(gocHt,i,d)   ;
  else if (y>-1 && y< 1) {
    vt = 0;
    Tg=millis();
  }
...
//  d=p-pe;
  ...
  float vt;
  if (gocHt<=70){
  if (y > 1  || y < -1 ) vt = gocHt*gocHt*gocHt/2  ;
  //if (y > 1  || y < -1 ) vt = PID(gocHt,i,d) ;
  else if (y>-1 && y< 1) vt = 0;
  if (vt > 255) vt=255;
  else if (vt<120) vt=0;
  else vt=vt-25;
  } 
